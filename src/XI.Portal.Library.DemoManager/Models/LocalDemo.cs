        ...
        public LocalDemo(string path, GameType version)
        {
            ...
            try
            {
                using (var stream = Open())
                {
                    ...
                    string map, mod, gameType, server, iwds, ffs;

                    config.TryGetValue("mapname", out map);
                    config.TryGetValue("fs_game", out mod);
                    config.TryGetValue("g_gametype", out gameType);
                    config.TryGetValue("sv_hostname", out server);
                    config.TryGetValue("sv_referencedIwdNames", out iwds);
                    config.TryGetValue("sv_referencedFFNames", out ffs);
        ...
        public string Name
        {
            ...
            set
            {
                var newPath = System.IO.Path.Combine(System.IO.Path.GetDirectoryName(Path),
                    string.Format("{0}{1}", value, System.IO.Path.GetExtension(Path)));
